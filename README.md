...
The steps below rebuild the corpus and the compiled search engine from
source; they require Python 3.11+ and Xcode command line tools (for the
Metal/C++ build), neither of which are needed just to install and use the
published CLI.
...
You'll also need a Hugging Face access token (free) to avoid API rate
